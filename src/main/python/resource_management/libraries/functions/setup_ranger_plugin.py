...
def setup_ranger_plugin(component_select_name, service_name, previous_jdbc_jar,
  ...
  file_path = format('{stack_root}/{stack_version}/usr/lib/ranger-{service_name}-plugin/install.properties')
  ...
  cmd_env = {'JAVA_HOME': java_home,
             'PWD': format('{stack_root}/{stack_version}/usr/lib/ranger-{service_name}-plugin'),
             'PATH': format('{stack_root}/{stack_version}/usr/lib/ranger-{service_name}-plugin')}
