...
xpack_security_enabled = params_json['es']['xpack_security_enabled']
auth_user = params_json['es']['auth_user']
auth_password = params_json['es']['auth_password']
...
def get_api_data():
  try:
    ...
    #     s.auth = (auth_user, auth_password)
    #
    base64string = base64.encodestring('%s:%s' % (auth_user, auth_password)).replace('\n', '')

    stats_request = urllib2.Request(es_cluster_stats_api)
    health_request = urllib2.Request(es_cluster_health_api)
    if xpack_security_enabled:
      stats_request.add_header("Authorization", "Basic %s" % base64string)
      health_request.add_header("Authorization", "Basic %s" % base64string)
    #
    logging.info("elasticsearch cluster stats api: {0}".format(es_cluster_stats_api))
    stats_resp = urllib2.urlopen(stats_request)
    #
    stats_content = json.loads(stats_resp.read().decode())
    ...
    #
    health_resp = urllib2.urlopen(health_request)
    health_content = json.loads(health_resp.read().decode())
...
def send_metric_to_collector(metric_name, metric_data):
  ...
  try:
    ...
    req = urllib2.Request(metrics_collector_api)
    req.add_header(*user_agent)
    resp = urllib2.urlopen(req, json.dumps(metrics_json))
    # resp = urllib2.urlopen(metrics_collector_api, json=metrics_json, headers=header)
    logging.info("send metrics result: {0}".format(resp.read().decode()))
  except Exception as e:
