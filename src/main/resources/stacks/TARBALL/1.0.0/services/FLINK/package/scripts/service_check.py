...
class FlinkServiceCheck(Script):
  def service_check(self, env):
    ...
    job_cmd_opts = " run-application -t yarn-application -Djobmanager.memory.process.size=1024m -Dtaskmanager.memory.process.size=1024m "
    run_flink_wordcount_job = format(
      "export HADOOP_CLASSPATH=`hadoop classpath`;{params.flink_bin_dir}/flink {job_cmd_opts} {params.flink_home}/examples/batch/WordCount.jar")
