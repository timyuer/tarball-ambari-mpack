...
class ServiceCheck(Script):
  def service_check(self, env):
    ...
    topic_exists_cmd = format(source_cmd + " ; " + "{kafka_home}/bin/kafka-topics.sh --bootstrap-server {kafka_connect} --topic {topic} --list")
    ...
    if topic not in topic_exists_cmd_out:
      create_topic_cmd = format("{kafka_home}/bin/kafka-topics.sh --bootstrap-server {kafka_connect} --create --topic {topic} --partitions 1 --replication-factor 1")
    ...
    under_rep_cmd = format("{kafka_home}/bin/kafka-topics.sh --describe --bootstrap-server {kafka_connect} --under-replicated-partitions")
