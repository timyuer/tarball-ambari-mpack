...
kafka_home = os.path.join(stack_version_home, "kafka")
...
# obtain the listener configs and extract the port
listeners = config['configurations']['kafka-broker']['listeners']
port = listeners.split(':')[-1]

# add the port for each host
kafka_hosts_with_ports = [host + ':' + port for host in kafka_hosts]

kafka_connect = ','.join(kafka_hosts_with_ports)

# get the security config for kafka
